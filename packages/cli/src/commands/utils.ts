...
import { z } from 'zod';
import { InvalidOptionsError } from '@/utils/errors';
import { safeValidate } from '@/utils/zod';

export const TRACE_ENV_VAR = 'LABEL_TRACE';
 ...
 * @returns
 */
export async function tryCommand<T, E extends LabelError>(
	...
	try {
		...
		if (e instanceof LabelError) error(e);
		error(
			new LabelError(e instanceof Error ? e.message : String(e), {
